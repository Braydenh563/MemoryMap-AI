...
- On a clean page at 1440x900, 1184x760 and 2000x1140 every step of every
  ...
  whether started from the Dashboard, Graph, Library or Reminders, with 40
  notes, with every API call slowed by 2.5s, with real scrollbars and at
  device scale 1.25. The failure was not in that path.
...
- One stale sentence: "Nothing you write is ever sent anywhere" against the
  welcome's own careful wording (web search and the update check can send).

## Done

Commit a052366 plus the copy and docs commit after it. See the commit
messages and CHANGELOG; the sweep `scratchpad/ui-sweeps/tour.js` walks every
step of every section at the three sizes plus the overlay, resize, typing,
double-Next, door and dim cases.

## Remaining

- Not verified: the owner's own window (Windows WebView2, his notebook, his
  zoom and theme). If "broken" recurs, ask which door he used and what was
  open; the overlay case is the one this pass found and fixed.
- The orchestrator: HISTORY record and moving this file to the archive at
  merge.
