...
| --- | --- | --- | --- | --- |
| FastAPI + SQLite (SQLAlchemy, WAL) in one local process | **Keep.** Right for a single-user offline notebook; ARCHITECTURE section 8 and 13 argue it well, and every competitor worth copying (Obsidian's index, Logseq DB) converged on SQLite | Nothing better; keep FTS5 and the vector table in the same file | none | |
