...
| More integration between features (INBOX 393) | Done: Ask Atlas about this (one icon, `ph:chat-circle`) on notes, documents, boards, files, reminders, web results and a text selection; Show in graph on notes (documents and boards reach the graph through its Documents and Boards switches); Open its note on reminders |
