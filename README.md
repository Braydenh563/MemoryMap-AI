...
| **Notes**     | Capture, browse and ask, as three sub-tabs. Auto-filing, tags, pins, threads, attachments, private notes (encrypted at rest), a recycle bin, revision history, a search box that understands `tag:work`, `cat:recipes`, `is:pinned`, `"exact phrase"` and `-exclude`, and **extract notes** — turn a block of free text (a draft, a document, a graph selection) into one or more AI-drafted, auto-linked notes, previewed before anything is saved |
