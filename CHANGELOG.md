...
- Documents, focus mode: "Fill the whole screen" fills it in the desktop window (it asks the window itself; the page's own full screen only filled the page). In a narrow suggestions panel the header's buttons are one row of icons with their names on hover, beside the count.
