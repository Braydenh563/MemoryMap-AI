...
#: Names the fault and the way out of it, in that order. The tab used to say
#: only the fault, and only on a disabled button's `title`: measured on the
#: writing room before D16, the section drew no offline row at all, so the one
#: click that fixes this (Settings, Models) was reachable from a tooltip on a
