...
// GRAPH-SIM-BEGIN
//: **Similarity is a backbone, not every pair** (INBOX 407, the owner: "when
...
function gcDraw(s = gcTab) {
  ...
  //: beneath every link (INBOX 407: the server appends them after the links,
...
//: The note's similarity lines as a list, strongest first (INBOX 407): the
...
function gcStartWorker(nodes, edges, world, s = gcTab, viewSeed = null) {
  ...
  gcPost({
    ...
    edges: edges.map((e) => ({
      ...
      //: changed nothing on this renderer (INBOX 407, measured).
...
async function renderGraphCanvas(s = gcTab) {
  ...
  //: same backbone (INBOX 407; see `gcPruneSimilarity`). After the hidden
...
//: **What the two kinds of line mean, said on the map** (INBOX 407). Only
