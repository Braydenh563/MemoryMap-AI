...
(async () => {
  ...
  const rows = () => page.evaluate(() => {
    ...
    const composeRow = document.querySelector('.draft-refine-row').getBoundingClientRect();
    return { compose: m('.draft-refine-row'), tags: m('.draft-tags-row'), saveRight: Math.round(save.right), tagsRight: Math.round(tagsRow.right), draftItRight: Math.round(draftIt.right), composeRight: Math.round(composeRow.right), actionsJustify: getComputedStyle(actions).justifyContent };
  });
