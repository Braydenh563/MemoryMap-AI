...
| A swipe on a phone row | `initRowSwipe` (app.js), touch only, below 600: the row's children slide on `--swipe-x`, the row's own `::before` (Favourite, `--accent-soft`) and `::after` (Bin, `--error-soft`) fill the gap, the underlay saturates past `ROW_SWIPE_ARM` (88px), and a lift-off past it presses the row's own control: the star (`.favourite-btn`) or the menu's own `binNoteWithUndo`. A swipe is never the only way to an action and never a second copy of one; `touch-action: pan-y` leaves the vertical drag to the page | `tests/test_ui_recipes.py`, `scratchpad/ui-sweeps/phoneswipe.js` |
