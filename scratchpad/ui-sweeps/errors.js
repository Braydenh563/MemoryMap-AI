...
// horizontal overflow and clipped cards at three widths. Prints only what is
...
(async()=>{
  ...
  for(const width of [1440,1024,390]){
    ...
    const findings=[];
    const check=async(label)=>{const r=await page.evaluate(()=>{const vis=e=>e.checkVisibility&&e.checkVisibility();const over=document.documentElement.scrollWidth>window.innerWidth+1;const clipped=[...document.querySelectorAll('.card, .tab-page:not(.hidden) button, .tab-page:not(.hidden) h2, .tab-page:not(.hidden) h3')].filter(e=>vis(e)&&getComputedStyle(e).overflow!=='visible'&&e.scrollHeight>e.clientHeight+2&&!e.matches('textarea, .tab-page, [class*="scroll"], .entry-list, ul, ol')).slice(0,4).map(e=>`${e.tagName.toLowerCase()}#${e.id}.${[...e.classList].slice(0,2).join('.')} ${e.scrollHeight}>${e.clientHeight}`);const off=[...document.querySelectorAll('.tab-page:not(.hidden) button, .tab-page:not(.hidden) input')].filter(e=>vis(e)).filter(e=>{const r=e.getBoundingClientRect();return r.right>window.innerWidth+1||r.left<-1;}).slice(0,4).map(e=>`${e.tagName.toLowerCase()}#${e.id}.${[...e.classList].slice(0,2).join('.')}`);return {over,clipped,off};});
    ...
    for(const t of TABS){where=t; await page.click(`[data-tab="${t}"]`).catch(()=>{}); await page.waitForTimeout(600); await check(t);
