...
| 2 | **The document/file editor** ([§R7.1](roadmap/REDESIGN.md)) | The largest single gap. **Items 1, 2 and 4 are built** — selection → chat context from every editing surface (each paragraph of a document's live view included); editing a text or code file in place, with `docview.editability` refusing a .docx, an over-long file or an unknown type in words written to be shown; and an HTML preview pane in a scriptless sandbox, served from `/files/{id}/html-preview` with its own CSP because a `blob:` frame inherits the app's and rendered the page unstyled. Items 3, 5 and 6 remain: syntax highlighting, per-format export, and opening a file in the editor from the Library rather than only a lightbox. All three are written up in [roadmap/HANDOVER.md](roadmap/HANDOVER.md). |
